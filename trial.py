...
for j in range(2):
    ...
    outline = circle.line
    outline.color.rgb = COLORS["white"]
    left1+=3.5
...
runs=p.runs[0]
runs.font.color.rgb = COLORS["black"]
...
for i in range(9):
    if (i%3==0): 
        ...
        p= tf.paragraphs[0]
        runs=p.runs[0]
        runs.font.color.rgb = COLORS["white"] #setting the font color to white
    elif (i%3==1):
...
fill=rectangle.fill
fill.solid()
fill.fore_color.rgb = COLORS["gold"]
outline = rectangle.line
outline.color.rgb = COLORS["gold"]
# 2- Textbox
...
fill.solid()
fill.fore_color.rgb = COLORS["flameOrange"] #dark red color
...
fill.fore_color.rgb = COLORS["black"] #setting the color of the line to black
# To color the border of the line 
outline = line.line
outline.color.rgb = COLORS["black"] #setting the color of the border to black
...
for i, (year,description) in enumerate(events):  #enumerate is used so that we can get the index of the event along with the item
    ...
    p.font.size = Pt(14) 
    p.font.bold = True
    p.font.color.rgb = COLORS["black"]  #setting the font color to white
...
fill= circle.fill
fill.solid()
fill.fore_color.rgb = COLORS["red"] 
outline = circle.line
outline.color.rgb = COLORS["red"] 
# Adding title 
...
fill = rectangle.fill
fill.solid()
fill.fore_color.rgb = COLORS["black"] #black
outline = rectangle.line
outline.color.rgb = COLORS["black"] #black

circle = slide4.shapes.add_shape(9, Inches(4.2), Inches(4), Inches(3), Inches(3)) 
fill= circle.fill
fill.solid()
fill.fore_color.rgb = COLORS["flameOrange"] 
outline = circle.line
outline.color.rgb = COLORS["flameOrange"]  
...
content_box.fill.fore_color.rgb = COLORS["burntOrange"]  
content_box.line.color.rgb = COLORS["burntOrange"] #Same color border        
...
subtitle_font.color.rgb = COLORS["black"]  # Black text
...
for i, data in enumerate(card_data):
    ...
    title_fill.solid()
    title_fill.fore_color.rgb = COLORS["pumpkinOrange"]  
    ...
    if data["has_player"]:
        ...
        player_font.color.rgb = COLORS["black"]
    ...
    content_fill.solid()
    content_fill.fore_color.rgb = COLORS["pumpkinOrange"]  # Orange background
...
desc_paragraph.font.color.rgb = COLORS["darkGray"]  
...
for row_idx, row_data in enumerate(table_data):
        for col_idx, cell_data in enumerate(row_data):
            ...
            if row_idx == 0:
                # Header styling
                cell.fill.solid()
                cell.fill.fore_color.rgb = COLORS["tangerine"] # Orange
                paragraph = cell.text_frame.paragraphs[0]
                paragraph.font.bold = True
                paragraph.font.size = Pt(15)
                paragraph.font.color.rgb = COLORS["white"]  # White text
                paragraph.alignment = PP_ALIGN.CENTER
            else:
                # Data row styling
                cell.fill.solid()
                if col_idx == 0:  # Feature name column
                    cell.fill.fore_color.rgb = COLORS["tangerine"]  # Light orange
                    paragraph = cell.text_frame.paragraphs[0]
                    paragraph.font.bold = True
                    ...
                    if "✓" in cell_data:
                        ...
                        paragraph.font.color.rgb = COLORS["darkGray"]  # Dark gray
...
container.fill.fore_color.rgb = COLORS["fireBrick"] 
container.line.color.rgb= COLORS["fireBrick"]    
...
for i, link in enumerate(links):
    ...
    rect.fill.solid()
    rect.fill.fore_color.rgb = COLORS["darkRed"] 
    rect.line.color.rgb = COLORS["darkRed"] 
    ...
    circle.fill.solid()
    circle.fill.fore_color.rgb = COLORS["oxblood"]
    circle.line.color.rgb=COLORS["oxblood"]

    # Adding the numbers
    circle_text = circle.text_frame
    circle_text.text = str(i+1)
    circle_text.paragraphs[0].font.color.rgb = COLORS["white"]
